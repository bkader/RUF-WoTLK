...
local function ElementEnable(self)
	...
	if element.PostUpdateVisibility then
		element:PostUpdateVisibility(true, not element.__isEnabled)
	end
...
local function ElementDisable(self)
	...
	if element.PostUpdateVisibility then
		element:PostUpdateVisibility(false, element.__isEnabled)
	end
...
local function Visibility(self, event, unit)
	...
	if (not UnitHasVehicleUI(unit)) then
		if ((UnitPowerType(unit) == ADDITIONAL_POWER_BAR_INDEX) and UnitPowerMax(unit, ADDITIONAL_POWER_BAR_INDEX) ~= 0) then
			shouldEnable = true
		end
	end
